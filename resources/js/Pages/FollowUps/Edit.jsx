...
export default function Edit({ auth, followUp, users }) {
    const timeZone = 'Asia/Kolkata';

    // Convert UTC time to IST for the form
    const followUpDate = parseISO(followUp.follow_up_at);
    const istDateTime = formatInTimeZone(followUpDate, timeZone, "yyyy-MM-dd'T'HH:mm");

    const { data, setData, put, processing, errors } = useForm({
        user_id: followUp.user_id || '',
        message: followUp.message || '',
        follow_up_at: istDateTime,
    });
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Edit Follow-up</h2>}
        >
